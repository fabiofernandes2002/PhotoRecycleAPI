...
exports.findAllDesafios = async (req, res) => {
  ...
  try {
    let data = await Desafio.find(condition).select('nome descricao recompensa').exec();
    res.status(200).json({
      success: true,
      desafios: data,
    });
...
exports.findOneDesafio = async (req, res) => {
  ...
  try {
    const desafio = await Desafio.findById(idDesafio).select('nome descricao recompensa').exec();
    if (desafio === null) {
