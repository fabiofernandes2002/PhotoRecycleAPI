...
exports.findAllMedalhas = async (req, res) => {
  ...
  try {
    const data = await Medalha.find(condition).select('nomeMedalha urlMedalha pontos').exec();
    res.status(200).json({
      success: true,
      medalhas: data,
    });
...
exports.findOneMedalha = async (req, res) => {
  ...
  try {
    ...
    const data = await Medalha.findById(idMedalha).select('nomeMedalha urlMedalha pontos').exec();
    if (!data)
      res.status(404).json({
        success: false,
